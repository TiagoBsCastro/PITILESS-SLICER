...
mf0 = mf("../TestRuns/pinocchio.{0:5.4f}.example.catalog.out".format(z), nsnap=snap, boxsize=boxsize)
...
k = np.geomspace(kmin, 1e1)
sol = HM.fitConc(k, z, mf0, cosmology.hmf_t10)

Pkw13 = HM.P1H(k, z, mf0, model='generic', A=sol.x[0], B=sol.x[1], Mpv=sol.x[2])
